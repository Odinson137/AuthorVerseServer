...
namespace AuthorVerseServer.Services
{
    public class MailService
    {
        ...
        public async Task<string> SendEmail(string jwtToken, string mail)
        {
            ...
            message.Body = new BodyBuilder() { HtmlBody = $"<a href = \"http://localhost:3000/AuthorVerse/EmailConfirm?token={jwtToken}\">Для подтверждения почты перейдите по ссылке {jwtToken}</a>" }.ToMessageBody();
